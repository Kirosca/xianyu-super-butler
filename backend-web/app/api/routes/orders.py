    ...
    for order in orders:
        ...
        payload.append(
            OrderOut(
                ...
                is_unregistered=order.is_unregistered or False,
                unregister_error_reason=order.unregister_error_reason or None,
...
async def no_logistics_delivery(
    ...
    acc_data = status_result.get("data", {}) if isinstance(status_result.get("data"), dict) else {}
    if not status_result.get("success") or not (acc_data.get("is_connected") or acc_data.get("running")):
