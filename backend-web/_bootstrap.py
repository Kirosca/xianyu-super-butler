...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"数据库生效URL: {settings.async_database_url}")
    ...
    try:
        if "sqlite" in settings.async_database_url:
            ...
            import common.models._exports
            from common.models.system_setting import SystemSetting
            from common.models.user import User, UserRole, UserStatus
            from common.utils.security import get_password_hash, generate_secret_key
            ...
            try:
                ...
                subprocess.run([sys.executable, str(Path(__file__).resolve().parent.parent / "init_all_sqlite_tables.py")], check=False)
            except Exception as e:
                logger.warning(f"全量 SQLite 建表异常: {e}")
            
            DEFAULT_SETTINGS = {
                'disclaimer.title': '免责声明',
                'disclaimer.content': '本软件仅供学习和交流使用，请勿用于非法用途。',
                'disclaimer.checkbox_text': '我已阅读并同意免责声明',
                'disclaimer.agree_button_text': '同意并继续',
                'disclaimer.disagree_button_text': '不同意',
                'login.system_name': '闲鱼自动回复系统',
                'login.system_title': '闲鱼自动回复系统',
                'login.system_description': '智能化闲鱼客服与卡券自动发货管理平台',
                'auth.footer_ad_html': '',
                'theme.effect': 'none',
                'theme.color_preset': 'blue',
                'theme.font_family': 'system-ui',
                'log.retention_days': '30',
                'account.face_verify_timeout_disable': 'false',
                'proxy.api_url': '',
                'proxy.enabled': 'false',
                'user.renew_month_price': '0',
                'user.register_default_days': '365',
                'captcha.real_mouse_weight_local': '1.0',
                'captcha.real_mouse_weight_remote': '1.0',
                'captcha.block_remote_calls': 'false',
                'captcha.local_slider_disabled': 'false',
                'captcha.remote_processing_max': '5',
                'captcha.remote_cooldown_seconds': '60',
                'captcha.remote_cooldown_until': '0',
                'captcha.slider_mode': 'stealth',
                'password_login.mode': 'normal',
                'token.api_mode': 'local',
                'token.remote_url': '',
            ...
            async with async_session_maker() as session:
                for key, val in DEFAULT_SETTINGS.items():
                    ...
                    if not setting:
                        session.add(SystemSetting(key=key, value=str(val), description="Default setting"))
            ...
            logger.info("SQLite 数据库结构与初始化完成")
        else:
