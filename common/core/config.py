...
class BaseConfig(BaseSettings):
    ...
    sync_driver: str = Field(default="sqlite")
    async_driver: str = Field(default="sqlite+aiosqlite")
    ...
    # 服务地址配置
    websocket_service_url: str = Field(default="http://127.0.0.1:8090")
    ...
    def database_url(self) -> str:
        """同步数据库连接URL"""
        if "sqlite" in self.sync_driver:
            db_path = Path(__file__).resolve().parent.parent.parent / "data" / "xianyu_data.db"
    ...
    def async_database_url(self) -> str:
        """异步数据库连接URL"""
        if "sqlite" in self.async_driver:
            db_path = Path(__file__).resolve().parent.parent.parent / "data" / "xianyu_data.db"
            db_path.parent.mkdir(parents=True, exist_ok=True)
