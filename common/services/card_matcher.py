...
class CardMatcher:
    ...
    async def update_card_item_relations(
        ...
        # 插入新关联
        is_sqlite = bool(self.session.bind and "sqlite" in str(self.session.bind.url))
        insert_cmd = "INSERT OR IGNORE" if is_sqlite else "INSERT IGNORE"
        added = 0
        for item_id in item_ids:
            ...
            await self.session.execute(
                text(f"""
                    {insert_cmd} INTO xy_card_item_relations 
                    (user_id, card_id, item_id, dock_record_id, created_at, updated_at)
                    VALUES (:user_id, :card_id, :item_id, 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                """),
        ...
        for rel in (card_relations or []):
            ...
            await self.session.execute(
                text("""
                    ...
                    VALUES (:user_id, :card_id, :item_id, :source, :dock_record_id, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                """),
                {"user_id": user_id, "card_id": card_id, "item_id": item_id,
